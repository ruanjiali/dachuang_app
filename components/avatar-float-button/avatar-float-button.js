Component({
  ...
  properties: {
    ...
    avatarSrc: {
      type: String,
      value: '/images/organization-avatar.png'
    },
    // 提示文本
    tooltipText: {
      type: String,
  ...
  methods: {
    ...
    updateButtonStyle: function() {
      ...
      this.setData({
        buttonStyle: `position: fixed; left: ${left}px; top: ${top}px;`
      });
    },
    ...
    handleTap() {
      ...
      // 已登录则导航到穿搭顾问页面
      wx.navigateTo({
        url: '/pages/clothes-advisor/clothes-advisor',
        fail: (err) => {
